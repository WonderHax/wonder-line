...
export class E2EE {
    ...
    async getE2EELocalPublicKey(mid, keyId) {
        ...
        if (toType === LINETypes.enums.MIDType.USER) {
            ...
            let receiverKeyData;
            if (!key) {
                receiverKeyData = await this.client.talk.negotiateE2EEPublicKey({ mid });
    ...
    async decryptE2EEMessage(messageObj) {
        if ((messageObj.contentType === "NONE" ||
        ...
        else if ((messageObj.contentType === "LOCATION" ||
    ...
    async __encryptAESCTR(aesKey, nonce, data) {
        ...
        return Buffer.from(await globalThis.crypto.subtle.encrypt({
            ...
            counter: nonce,
            length: 32,
        }, await globalThis.crypto.subtle.importKey("raw", new Uint8Array(aesKeyArrayBuffer), "AES-CTR", false, ["encrypt"]), new Uint8Array(dataArrayBuffer)));
    }
    ...
    async ___decryptAESCTR(aesKey, nonce, data) {
        ...
        // @ts-expect-error: will fix cuz typescript version change
        await globalThis.crypto.subtle.importKey("raw", aesKey, "AES-CTR", false, ["decrypt"]), data));
    }
