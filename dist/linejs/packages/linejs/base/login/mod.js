...
export class Login {
    ...
    async requestSQR() {
        ...
        if (await this.checkQrCodeVerified(sqr)) {
            ...
            if (e2eeInfo) {
                await this.client.e2ee.decodeE2EEKeyV1(e2eeInfo, Buffer.from(secret));
            }
            return authToken;
        }
    ...
    async requestEmailLogin(email, password, constantPincode = "114514", enableE2EE = true) {
        ...
        if (!response.authToken) {
            ...
            if (enableE2EE && secret) {
                ...
                const deviceSecret = this.client.e2ee.encryptDeviceSecret(Buffer.from(e2eeInfo.metadata.publicKey, "base64"), Buffer.from(secret), Buffer.from(e2eeInfo.metadata.encryptedKeyChain, "base64"));
    ...
    async getRSAKeyInfo(provider = 0) {
        return await this.client.request.request([[12, 1, [[8, 2, LINEStruct.IdentityProvider(provider)]]]], "getRSAKeyInfo", 3, "RSAKey", "/api/v3/TalkService.do");
    }
    ...
    async checkQrCodeVerified(qrcode) {
        try {
            await this.client.request.request([[12, 1, [[11, 1, qrcode]]]], "checkQrCodeVerified", 4, false, "/acct/lp/lgn/sq/v1", {
                "x-lst": "180000",
    ...
    async verifyCertificate(qrcode, cert) {
        return await this.client.request.request([[12, 1, [[11, 1, qrcode], [11, 2, cert]]]], "verifyCertificate", 4, false, "/acct/lgn/sq/v1");
    }
    ...
    async checkPinCodeVerified(qrcode) {
        try {
            await this.client.request.request([[12, 1, [[11, 1, qrcode]]]], "checkPinCodeVerified", 4, false, "/acct/lp/lgn/sq/v1", {
                "x-lst": "180000",
                "x-line-access": qrcode,
            }, this.client.config.longTimeout);
            return true;
    ...
    async qrCodeLogin(authSessionId, autoLoginIsRequired = true) {
        return await this.client.request.request([[12, 1, [
                    [11, 1, authSessionId],
