...
export class LineObs {
    constructor(client) {
        this.prefix = "https://obs.line-apps.com/";
        this.client = client;
    }
    /**
    ...
    async uploadObjectForService(options) {
        ...
        let headers = this.client.request
            .getHeader("POST");
    ...
    async uploadMediaByE2EE(options) {
        ...
        const { keyMaterial, encryptedData } = await this.client.e2ee
            .encryptByKeyMaterial(Buffer.from(await data.arrayBuffer()));
    ...
    async uploadMediaForSquare(options) {
        const { data, oType, to, filename } = options;
        if (to[0] !== "m") {
    ...
    async downloadMediaByE2EE(message) {
        ...
        const talkMeta = Buffer.from(JSON.stringify({
            message: Buffer.from(writeStruct([[11, 4, id], [15, 27, [12, []]]], thrift.TBinaryProtocol)).toString("base64"),
        })).toString("base64");
