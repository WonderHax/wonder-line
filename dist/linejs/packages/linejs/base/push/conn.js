...
export class Conn {
    constructor(manager) {
        ...
        this.cacheData = new Uint8Array(0);
        this.notFinPayloads = {};
    ...
    async new(host, _port, path, headers = {}) {
    ...
    async writeRequest(requestType, data) {
    ...
    isAble2Request() {
        if (this.client.authToken && !this._closed) {
            if (Date.now() / 1000 - this._lastSendTime > 0.5)
                return true;
        }
        return false;
    }
    readPacketHeader(data) {
