...
// GOMI:
import { SquareService_fetchMyEvents_args as gen_SquareService_fetchMyEvents_args, sync_args as gen_sync_args, } from "../thrift/readwrite/struct.ts";
...
export class ConnManager {
    constructor(base) {
        ...
        this.opStream = this.createAsyncReadableStream();
        this.sqStream = this.createAsyncReadableStream();
    }
    log(text, data) {
    ...
    async initializeConn(state = 1, initServices = [3, 6, 8, 9, 10]) {
        ...
        const tosendHeaders = this.client.request
            .getHeader();
    ...
    async buildAndSendSignOnRequest(conn, serviceType, kwargs = {}) {
        ...
        if (serviceType === 3) {
            ...
            // @ts-ignore: will fix
            req = cl.thrift.writeThrift(gen_SquareService_fetchMyEvents_args(kwargs), "fetchMyEvents", TCompactProtocol);
            methodName = "fetchMyEvents";
        }
        else if ([5, 8].includes(serviceType)) {
            // @ts-ignore: will fix
            req = cl.thrift.writeThrift(gen_sync_args(kwargs), "sync", TCompactProtocol);
            methodName = "sync";
        }
    ...
    async _OnSignOnResponse(reqId, isFin, data) {
        // data = data.slice(5);
        const cl = this.client;
        ...
        try {
            ...
            if (serviceType === 3) {
                const resp = cl.thrift.rename_data(cl.thrift.readThrift(data, TCompactProtocol), true).data;
                ...
                const { subscription: { subscriptionId }, events, syncToken } = resp.success;
    ...
    async _OnPushResponse(pushFrame) {
    ...
    async InitAndRead(initServices = [3, 5]) {
...
//# sourceMappingURL=connManager.js.map