import { Protocols, } from "../thrift/mod.ts";
...
export class RequestClient {
    ...
    async request(value, methodName, protocolType = 3, parse = true, path = "/S3", headers = {}, timeout = this.client.config.timeout) {
        const res = await this.requestCore(path, value, methodName, protocolType, headers, undefined, parse, undefined, timeout);
     ...
     */
    async requestCore(path, value, methodName, protocolType, appendHeaders = {}, overrideMethod = "POST", parse = true, isReRequest = false, timeout = this.client.config.timeout) {
        ...
        if (methodName === "joinSquare") {
            const hex = [...Trequest].map(b => b.toString(16).padStart(2, "0")).join(" ");
            this.client.log("joinSquare:requestHex", {
                length: Trequest.length,
                hex,
            });
        }
        ...
        if (methodName === "joinSquare") {
            const hex = [...parsedBody].map(b => b.toString(16).padStart(2, "0")).join(" ");
            this.client.log("joinSquare:responseHex", {
        ...
        try {
            ...
            throw new Error(`Request internal failed: Invalid response buffer <${[...parsedBody].map((e) => e.toString(16)).join(" ")}>`);
        }
        ...
        const isRefresh = Boolean(res.data.e &&
            res.data.e.code === "MUST_REFRESH_V3_TOKEN" &&
            await this.client.storage.get("refreshToken"));
        if (res.data.e && !isRefresh) {
            throw new InternalError("RequestError", `Request internal failed, ${methodName}(${path}) -> ` +
                JSON.stringify(res.data.e), res.data.e);
        }
