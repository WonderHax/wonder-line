...
export class LiffService {
    constructor(client) {
        ...
        this.csrfRegExp = /<input type="hidden" name="__csrf" id="__csrf" value="(.*?)">/;
        this.client = client;
    }
    /**
    ...
    async tryConsentAuthorize(consentUrl, channelId, allPermission = ["P", "CM", "OC"], approvedPermission = ["P", "CM", "OC"]) {
        const headers = {
            "X-Line-Access": this.client?.authToken,
        ...
        if (response.ok) {
            ...
            if (channelId && csrfToken) {
                ...
                const authResponse = await this.client.fetch(LiffService.AUTH_CONSENT_URL, {
                    method: "POST",
                    body: `${allPermission.map((e) => "allPermission=" + e)
                        .join("&")}&${approvedPermission.map((e) => "approvedPermission=" + e).join("&")}&__WLS=&channelId=2006747340&__csrf=${csrfToken}&allow=true`,
                    headers,
                });
...
LiffService.LINE_LIFF_ENDPOINT = "https://api.line.me/message/v3/share";
LiffService.CONSENT_API_URL = "https://access.line.me/dialog/api/permissions";
LiffService.AUTH_CONSENT_URL = "https://access.line.me/oauth2/v2.1/authorize/consent";
