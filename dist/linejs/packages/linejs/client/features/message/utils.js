 ...
 */
export const buildTextDecorations = (decorationText) => {
    let text = "";
    ...
    decorationText.forEach((e) => {
        if (e.emoji) {
            ...
            hasEmoji = true;
            const replace = JSON.parse(_contentMetadata.REPLACE);
            ...
            const sticon = JSON.parse(_contentMetadata.STICON_OWNERSHIP);
            ...
            hasMention = true;
            if (e.mention.all) {
                const mention = JSON.parse(_contentMetadata.MENTION);
                mention.MENTIONEES ?? (mention.MENTIONEES = []);
