...
function getFunctions(input) {
    ...
    const fReg = /"(?<name>.*?)"/g;
    const fReg2 = /'(?<name>.*?)'/g;
    const square = input.includes("class SquareLiveTalkService(BaseService):") ||
    ...
    while (true) {
        ...
        if (fn) {
            used.push((square ? "SquareService_" + result.groups.name : result.groups.name) +
                "_args");
            functions.push(fn);
        }
    }
...
function f(name, square = false) {
    ...
    ): Promise<${Thrift[name + "_result"] &&
        Thrift[name + "_result"].find((e) => e.name === "success")
...
for (const s of service) {
    const functions = getFunctions(await Deno.readTextFile(`../../../_CHRLINE/CHRLINE/services/${s}.py`));
    if (functions.length) {
...
//# sourceMappingURL=gen_service.js.map