...
export function main(Thrift = _Thrift, path) {
    const argList = [];
    const structList = [];
    const result = [`
    ...
    function field(data) {
        ...
        else if (typeof data.map === "string" && typeof data.key === "number" &&
            isExist(data.map)) {
    ...
    Deno.writeTextFileSync(path || path_.fromFileUrl(import.meta.resolve("../../packages/linejs/base/thrift/readwrite/struct.ts")), result.join("") + args.join(""));
}
//# sourceMappingURL=gen_struct.js.map