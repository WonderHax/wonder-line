...
export function main(Thrift = _Thrift, path) {
    ...
    const toType = (tname) => typeof tname !== "undefined"
        ? (tname !== "_any"
            ? (tname !== "_anykey"
                ? (typeof tname === "string"
                    ? (typeof Thrift[tname] !== "undefined" ? tname : "any")
                    : ttype[tname])
                : "keyof any")
            : "any")
        : "any";
    function toUnion(input, name) {
        const unions = [];
        const enumValue = {};
    ...
    function toInterface(input, name) {
        const defs = [];
        for (const e of input) {
            if (typeof e.type !== "undefined") {
                defs.push(`${e.name}: ${buf_replaces.includes(e.name) ? "(string|Buffer)" : toType(e.type)};`);
            }
            else if (typeof e.struct !== "undefined") {
                defs.push(`${e.name}: ${toType(e.struct)};`);
            }
            else if (typeof e.map !== "undefined" && typeof e.key !== "undefined") {
                defs.push(`${e.name}: ${ttype[13].replace("$K", e.key === 10 ? "number" : toType(e.key)).replace("$V", buf_replaces.includes(e.name) ? "(string|Buffer)" : toType(e.map))};`);
            }
            else if (typeof e.list !== "undefined") {
                defs.push(`${e.name}: ${ttype[14].replace("$E", buf_replaces.includes(e.name)
                    ? "(string|Buffer)"
                    : toType(e.list))};`);
            }
            else if (typeof e.set !== "undefined") {
                defs.push(`${e.name}: ${ttype[15].replace("$E", buf_replaces.includes(e.name) ? "(string|Buffer)" : toType(e.set))};`);
            }
        }
