...
import { MimeType } from "https://raw.githubusercontent.com/wonderhax/wonder-line/main/C:/Users/Admin/Documents/work/wonder-line/linejs/packages/linejs/base/obs/mime.ts";
...
import { writeStruct } from "https://raw.githubusercontent.com/wonderhax/wonder-line/main/C:/Users/Admin/Documents/work/wonder-line/linejs/packages/linejs/base/thrift/readwrite/write.ts";
...
export class LineObs {
	...
	public getMessageMetadataUrl(
		...
		return `${this.prefix}r/${square ? "g2" : "talk"
	...
	public async uploadObjTalk(
		...
		//	param.duration = "11516745000"
			param.duration = "9999999999999999999999999"; // 810
		}
	...
	public async uploadMediaForSquare(options: {
		data: Blob;
		...
		if (to[0] !== "m") {
			throw new Error("Invalid squareChatMid");
		}
		...
		const { objId } = await this.uploadObjectForService({
			data,
			oType: "file",
			obsPath: `talk/${obsNamespace}/${tempId}`, //obspath
			params: { type: "file" },
		});

		return await this.client.square.sendMessage({
			squareChatMid: to,
			contentType,
			contentMetadata: {
				SID: obsNamespace,
